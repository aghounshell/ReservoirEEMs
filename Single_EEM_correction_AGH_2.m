...
for i = 1:301
    for j = 1:85
        if (corr.Em(i)-10) <= corr.Ex(j)
            corr.test(i,j) = NaN;
        elseif corr.Em(i) ~= corr.Ex(j)
            corr.test(i,j) = corr.X(i,j);
        end
    end
end

% Remove 2nd order Rayleigh scattering
for i = 1:301
    for j = 1:85
        if (corr.Em(i)+10) >= 2*corr.Ex(j)
            corr.test2(i,j) = NaN;
        elseif corr.Em(i) ~= corr.Ex(j)
            corr.test2(i,j) = corr.test(i,j);
        end
    end
end
